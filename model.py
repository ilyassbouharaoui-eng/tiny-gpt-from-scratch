...
def softmax_overflow_demo(large_value):
    ...
    """
    d = {}
    val = array_exp(np.array(large_value))
    if np.isinf(val):
        d['naive_exp'] = 'inf'
        d['overflowed'] = True
    else:
...
def stable_softmax_1d(logits):
    ...
    max = max_along_axis(logits,axis = 0)
    x = logits - max
    return array_exp(x)/sum_all(array_exp(x))
...
def stable_softmax_2d_rowwise(logits):
    ...
    max = max_along_axis(logits,axis = 1).reshape((len(logits),1))
    x = logits - max
    return array_exp(x) / sum_keepdims(array_exp(x),axis = 1)
...
def encode_corpus_to_int_array(text, stoi):
    ...
    return np.array(encode_string(text,stoi),dtype = 'int64')
...
def sample_next_token(p_matrix, current_id, rng):
    ...
    pr = p_matrix[current_id]
    return int(rng.choice(range(len(p_matrix[0])) , p = pr))
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    current_id = start_id
    while i < length-1:
        x = sample_next_token(p_matrix,current_id,rng)
        l.append(x)
        current_id = x
        i+=1
    return np.array(l)
...
def sum_negative_log_probs(p_matrix, data):
    return -sum(log_prob_of_pair(p_matrix,data[i],data[i+1]) for i in range(len(data)-1))
...
def initialize_w_random(vocab_size, rng):
    ...
    return rng.standard_normal((vocab_size,vocab_size),dtype = 'float64')
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    M = make_2d_zeros(len(ids),vocab_size)
...
def observe_lookup_equivalence(w, ids):
    ...
    """
    on_hot = one_hot_encode_batch(ids,len(w))
    return {'onehot_result' : on_hot@w ,
    'index_result' : w[ids]}
...
def run_one_training_step(w, ids, targets, learning_rate):
    ...
    dw = compute_dw_scatter_add(ids, dlogits, len(w))
...
def linear_backward_dw(dy, cache):
    ...
    x = cache['x']
    return matmul(np.transpose(x),dy)
...
def layernorm_forward_affine(x, gamma, beta, eps):
    ...
    y = vector_matrix_broadcast_add(y,beta)
    return {'y' : y,
    'cache' : {'x' : x,
    'x_hat' : x_hat,
    'mean' : mean,
...
def layernorm_backward_subtract_mean(dy, cache):
    ...
    """
    return dy - layernorm_forward_mean(dy)
...
def layernorm_backward_full(dy, cache):
    ...
    dx = (1 / D) * (1 / sigma) * (
    D * dy * gamma
    - np.sum(dy * gamma, axis=1, keepdims=True)
    - x_hat * np.sum(dy * gamma * x_hat, axis=1, keepdims=True)
    )
    return {'dx': dx,
    'dgamma' : dgamma,
    'dbeta' : dbeta}
